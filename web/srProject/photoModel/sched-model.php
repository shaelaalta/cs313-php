...
function getFullSched(){
    $db = connect();
    $sql = 'SELECT usert.userfirstname, usert.userlastname, schedule.* FROM ((apt INNER JOIN usert ON apt.userid = usert.userid) INNER JOIN schedule ON apt.schedid = schedule.schedid AND schedule.booked = 2)';
    $stmt = $db->prepare($sql);
    $stmt->execute();
    $specific = $stmt->fetchAll(PDO::FETCH_ASSOC);
    $stmt->closeCursor();

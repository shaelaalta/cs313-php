...
function getPerson($sessId){
     $db = connect();
    $sql = 'SELECT usert.firstname, usert.lastname, schedule.* FROM ((apt INNER JOIN usert ON apt.userid = usert.userid AND apt.userid = :sessId) INNER JOIN schedule ON apt.schedid = schedule.schedid)';
    $stmt = $db->prepare($sql);
